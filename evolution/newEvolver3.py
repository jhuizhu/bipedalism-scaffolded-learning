  ...
  def mutateParams(self, inputParams):
    while True:
      ...
      for key in defaultParams:
        # if (binomial (1, parameterDic[key]['mutationProb']) and Ave >=4.8763):
        #   newParams[key] = saturate(parameterDic[key]['floor'], parameterDic[key]['ceil'], normal(newParams[key], parameterDic[key]['standardDev']))
        #   newParams['halfBodyX'] = saturate(parameterDic['halfBodyX']['floor'], round(parameterDic['halfBodyX']['ceil']*4.8763/Ave, 1), normal(newParams['halfBodyX'], parameterDic['halfBodyX']['standardDev']))
        # else:
        #   newParams[key] = saturate(parameterDic[key]['floor'], parameterDic[key]['ceil'], normal(newParams[key], parameterDic[key]['standardDev']))
        ...
        #   # print 'Bodylength out of range'
        
        if (binomial (1, parameterDic[key]['mutationProb'])):
          newParams[key] = saturate(parameterDic[key]['floor'], parameterDic[key]['ceil'], normal(newParams[key], parameterDic[key]['standardDev']))
          newParams['halfBodyX'] = saturate(parameterDic['halfBodyX']['floor'], max(parameterDic['halfBodyX']['ceil']*4.8763/Ave-0.2, 0), normal(newParams['halfBodyX'], parameterDic['halfBodyX']['standardDev']))

###Simulted annealing
      #global currentGen
      # global Ave
      # if (currentGen>=0 and currentGen<4000 and Ave >=4.8763):
      #   newParams['halfBodyX'] = saturate(parameterDic['halfBodyX']['floor'], parameterDic['halfBodyX']['ceil']*4.8763/Ave-0.2, normal(newParams['halfBodyX'], parameterDic['halfBodyX']['standardDev']))
      #   # newParams['halfBodyX'] = saturate(parameterDic['halfBodyX']['floor'], round(parameterDic['halfBodyX']['ceil']*4.8763/Ave, 1), normal(newParams['halfBodyX'], parameterDic['halfBodyX']['standardDev']))
      #   # print 'halfMaxBodyX1:' + str(parameterDic['halfBodyX']['ceil']*5.86886/Ave)  
      # # elif (currentGen>=1 and currentGen<5):
      # #   newParams['halfBodyX'] = saturate(parameterDic['halfBodyX']['floor'], parameterDic['halfBodyX']['ceil']*4.8763/Ave, normal(newParams['halfBodyX'], parameterDic['halfBodyX']['standardDev']))
      # else:
      #   newParams['halfBodyX'] = saturate(parameterDic['halfBodyX']['floor'], parameterDic['halfBodyX']['ceil'], normal(newParams['halfBodyX'], parameterDic['halfBodyX']['standardDev']))
      #   print 'Bodylength out of range'
###     
...
def main_function():
  ...
  while currentGen < maxGens:
    ...
    print 'halfMaxBodyX: ' + str(max(parameterDic['halfBodyX']['ceil']*4.8763/Ave-0.2, 0))
    # print '>>>' + str(round(parameterDic['halfBodyX']['ceil']*4.8763/Ave, 1))
    ...
    Ave = sum(overallFittest[:3])/3

    print 'AverageBest3 Fitness:' + str(Ave)
    summarisedOutput.write('AverageBest3 Fitness: ' + str(Ave) + '\n\n')
